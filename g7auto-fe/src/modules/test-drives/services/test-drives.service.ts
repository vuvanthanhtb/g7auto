...
class TestDrivesRepository implements ITestDrivesRepository {
  ...
  create(data: TestDriveRequest): AR<TestDriveResponse> {
    return http.call<TestDriveResponse>({ url: BASE, method: "POST", data });
  }
  confirm(id: number): AR<TestDriveResponse> {
    return http.call<TestDriveResponse>({
      url: `${BASE}/${id}/confirm`,
...
export const testDrivesService: ITestDrivesRepository =
  TestDrivesRepository.getInstance();
