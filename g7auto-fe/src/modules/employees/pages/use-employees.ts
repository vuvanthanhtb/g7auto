import { useEffect, useState } from "react";
import { useAppDispatch, useAppSelector } from "@/shell/redux/hooks";
import {
  getEmployees,
  createEmployee,
  updateEmployee,
  getEmployeeById,
  clearSelectedEmployee,
} from "../shell/employees.slice";
import { employeesService } from "../shell/employees.service";
import type { EmployeeRequest } from "../shell/employees.type";
import { employeeInitialValues } from "./employees.config";
import { BTN_SEARCH, BTN_REFRESH, BTN_EXPORT, BTN_EDIT, BTN_SUBMIT } from "@/libs/constants/button.constant";
import { normalizeFormValues } from "@/libs/utils";

type TableRow = Record<string, unknown>;

export const useEmployees = () => {
  const dispatch = useAppDispatch();
  const { selected } = useAppSelector((s) => s.employees);
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [editId, setEditId] = useState<number | null>(null);
  const [formValues, setFormValues] = useState<Record<string, unknown>>(employeeInitialValues);
  const [page, setPage] = useState(1);
  const [searchParams, setSearchParams] = useState<Record<string, unknown>>({});

  useEffect(() => {
    document.title = "Nhân viên — G7Auto";
    dispatch(getEmployees({ page, size: 10, ...searchParams }));
  }, [dispatch, page, searchParams]);

  useEffect(() => {
    if (selected && editId) setFormValues(selected as unknown as Record<string, unknown>);
  }, [selected, editId]);

  const openCreate = () => {
    setEditId(null);
    setFormValues(employeeInitialValues);
    setDrawerOpen(true);
  };

  const closeDrawer = () => {
    setDrawerOpen(false);
    dispatch(clearSelectedEmployee());
  };

  const handleCellAction = (row: TableRow, key?: string) => {
    if (key === BTN_EDIT) {
      setEditId(row.id as number);
      dispatch(getEmployeeById(row.id as number));
      setDrawerOpen(true);
    }
  };

  const handleSubmit = async (data: Record<string, unknown>) => {
    if (editId) await dispatch(updateEmployee({ id: editId, data: data as unknown as EmployeeRequest }));
    else await dispatch(createEmployee(data as unknown as EmployeeRequest));
    closeDrawer();
    dispatch(getEmployees({ page, size: 10, ...searchParams }));
  };

  const searchHandlers = {
    [BTN_SEARCH]: (values: Record<string, unknown>) => { setSearchParams(normalizeFormValues(values)); setPage(1); },
    [BTN_REFRESH]: () => { dispatch(getEmployees({ page, size: 10, ...searchParams })); },
    [BTN_EXPORT]: async () => { await employeesService.export(); },
  };

  const formHandlers = { [BTN_SUBMIT]: handleSubmit };

  return {
    drawerOpen, editId, formValues, page, searchParams,
    openCreate, closeDrawer, handleCellAction, searchHandlers, formHandlers,
    setFormValues, setPage,
  };
};
