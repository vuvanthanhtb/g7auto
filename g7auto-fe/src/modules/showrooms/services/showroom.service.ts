...
class ShowroomRepository implements IShowroomRepository {
  ...
  getById(id: number): AR<ShowroomResponse> {
    return http.call<ShowroomResponse>({ url: `${BASE}/${id}`, method: "GET" });
  }
  create(data: ShowroomRequest): AR<ShowroomResponse> {
    return http.call<ShowroomResponse>({ url: BASE, method: "POST", data });
  }
  update(id: number, data: ShowroomRequest): AR<ShowroomResponse> {
    return http.call<ShowroomResponse>({
      url: `${BASE}/${id}`,
...
export const showroomService: IShowroomRepository =
  ShowroomRepository.getInstance();
