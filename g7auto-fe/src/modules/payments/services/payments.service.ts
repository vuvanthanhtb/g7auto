...
import type { PaymentRequest, PaymentPayload, PaymentResponse } from "../shell/payments.type";
...
class PaymentsRepository implements IPaymentsRepository {
  ...
  getById(id: number): AR<PaymentResponse> {
    return http.call<PaymentResponse>({ url: `${BASE}/${id}`, method: "GET" });
  }
  create(data: PaymentRequest): AR<PaymentResponse> {
    return http.call<PaymentResponse>({ url: BASE, method: "POST", data });
  }
  confirm(id: number): AR<PaymentResponse> {
    return http.call<PaymentResponse>({ url: `${BASE}/${id}/confirm`, method: "POST" });
  }
  cancel(id: number): AR<PaymentResponse> {
    return http.call<PaymentResponse>({ url: `${BASE}/${id}/cancel`, method: "POST" });
  }
  exportExcel(): Promise<void> {
    return http.download({ url: `${BASE}/export`, filename: "danh-sach-thanh-toan.xlsx" });
  }
}
