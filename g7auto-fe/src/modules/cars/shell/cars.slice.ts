...
export const getCars = createAsyncThunk("cars/getList", async (params: CarQuery) => {
  const res = await carsService.getList(params);
  return res.data;
});

export const getCarById = createAsyncThunk("cars/getById", async (id: number) => {
  const res = await carsService.getById(id);
  return res.data;
});

export const createCar = createAsyncThunk("cars/create", async (data: CarRequest) => {
  const res = await carsService.create(data);
  return res.data;
});

export const updateCar = createAsyncThunk("cars/update", async ({ id, data }: { id: number; data: CarUpdateRequest }) => {
